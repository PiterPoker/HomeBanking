...
{
    public class GetInvitationsByUserIdCommandHandler
        : IRequestHandler<GetInvitationsByUserIdCommand, PaginatedItemsViewModel<FamilyDTO>>
    {
        private readonly FamilyDomain.IFamilyRepository _familyRepository;

        public GetInvitationsByUserIdCommandHandler(FamilyDomain.IFamilyRepository familyRepository)
        {
            _familyRepository = familyRepository ?? throw new ArgumentNullException(nameof(familyRepository));
        }

        public async Task<PaginatedItemsViewModel<FamilyDTO>> Handle(GetInvitationsByUserIdCommand command, CancellationToken cancellationToken)
        {
            var families = await _familyRepository.GetAsync();

            var familiesByUserId = families.Where(f => f.Invitations.Any(r => r.EqualsUserById(command.UserId)));
